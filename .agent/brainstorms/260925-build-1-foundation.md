...
> **Status**: Accepted — Implemented by [Build 1 Foundation](../plans/260925-build-1-foundation.md)
> **Created**: 2026-09-25
...
| 2026-09-26 | **Amended during hosted verification:** authorize the two fixed backoffice operators by normalized Google email, provided the verified JWT contains `email_verified: true`; supersede the earlier `(issuer, subject)` allowlist only for this backoffice. | Andrew prefers manually maintaining the two familiar addresses and will request a configuration change if either account changes. Convex still verifies issuer, audience, signature and expiry, arbitrary Google users remain denied, and future Business-user identity continues to use its separately designed stable identifiers. |
| 2026-09-26 | Keep the two operator emails in source rather than deployment environment variables. | Andrew wants intentionally identical configuration to be defined once in code across every development and production deployment. He accepts that the addresses are visible to repository readers and that changing the list requires a tested BFF deployment. |
| 2026-09-26 | Centralize deployment-invariant, non-secret identifiers in one internal static-configuration library; keep ordinary constants local and environment-specific values external. | The shared Google client ID and two operator addresses are identifiers rather than credentials. One reviewed source prevents repeated per-environment setup, while browser bundle validation prevents the operator list from being shipped accidentally. |
